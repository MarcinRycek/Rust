...
impl Rgb {
    ...
    fn from_3percent(r: f64, g: f64, b: f64) -> Result<Self, String> {
        if !(0.0..=100.0).contains(&r) || !(0.0..=100.0).contains(&g) || !(0.0..=100.0).contains(&b) {
            return Err("Error: values must be: 0.0 - 100.0".to_string());
        }
        Ok(Rgb {
            r: (255.0 * r / 100.0).round() as u8,
            g: (255.0 * g / 100.0).round() as u8,
            b: (255.0 * b / 100.0).round() as u8,
        })
    }

    fn gray(n: f64) -> Result<Self, String> {
        Self::from_3percent(n, n, n)
    }
...
fn main() {
    ...
    println!("{:.5}", fiolet.intensity() == 1.0 / 3.0);
